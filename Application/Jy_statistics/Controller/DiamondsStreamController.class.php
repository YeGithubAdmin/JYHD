...
class DiamondsStreamController extends ComController {
    //列表
    public function index(){
        ...
        $where = ' CurrencyType = 2 and  DateTime < str_to_date("'.$EndTime.'","%Y-%m-%d %H:%i:%s")  and DateTime >= str_to_date("'.$StartTime.'","%Y-%m-%d %H:%i:%s")';
        if ($search['Income'] != 0){
