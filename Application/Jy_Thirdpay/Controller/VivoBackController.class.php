...
use Think\Model;
class VivoBackController extends Controller {
    ...
    public function index(){
        ...
        $parseResp = $VivoBack->Verification($dataThirdpay,$dataThirdpay['signature'],'55a40a6528176fff6c31fc7da3bf73c7');
        if(!$parseResp){
            $result = 7002;
            goto OrderSave;
        }
        ...
        //支付状态  0 成功  1  失败
        if($OrderInfo['tradeStatus'] != 0000 && !isset($OrderInfo['tradeStatus'])){
            $result = 7003;
            goto failed;
        }
        //订单号
        ...
        $paytype        = 0;
        //金额
        $money          =   $OrderInfo['orderAmount']/100;
        /*************************以下通用****************************/
        //实例化数据
        $model = new Model();
        //查询订单信息
