...
class HuaWeiController extends Controller {
    public function index(){
        ...
        $result = 2001;
        if($dataThirdpay == null || $dataThirdpay === ''){
            $result = 4001;
            goto  failed;
        }
        ...
        //验签
        $PayVerification = $HuaWeiFun->PayVerification($valueMap,$Sign);
        if(!$PayVerification){
            $result = 7002 ;
            goto  failed;
        }
        ...
        if(C('ACCESS_lOGS')){
            $dir = C('YQ_ROOT').'Log/pay/'.date('Y').'/'.date('m').'/'.date('d').'/';
            $ObjFun->record_log($dir,'access_'.date('Ymd').'.log',json_encode($valueMap));
        }
        //订单号
        $OrderID        = $valueMap['requestId'];
        //金额
        $money          = $valueMap['amount'];
        //实例化数据
        $model = new Model();
        //查询订单信息
