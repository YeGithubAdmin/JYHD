...
class LenovoController extends Controller {
    public function index(){
        include LENOVOSDK.'/lib/lenovo_cashier_rsa.inc.php';
        $ObjFun = new \Common\Lib\func();
        $transdata = trim($_POST['transdata']);
        if(C('ACCESS_lOGS')){
            $dir = C('YQ_ROOT').'Log/pay/'.date('Y').'/'.date('m').'/'.date('d').'/';
            $ObjFun->record_log($dir,'access_'.date('Ymd').'.log',$transdata);
        }
        $dataThirdpay = json_decode($transdata,true);
        $msgArr = array(
            2001=>'支付成功！',
            3000=>'支付并发',
            3001=>'服务器无响应！',
        ...
        if($dataThirdpay['result'] == 1){
            $result = 7003;
            goto  failed;
        }
        //订单号
        $OrderID        = $dataThirdpay['exorderno'];
        //金额
        $money          = $dataThirdpay['money'];
        //实例化数据
        $model = new Model();
        //查询订单信息
        ...
        $priKey = '';
        $SignLocal = sign($transdata, $priKey);
        //验签
        $Rsaverify = verify($transdata, $priKey, $SignLocal);
        if(!$Rsaverify){
            $result = 7002;
            goto OrderSave;
        }
