...
use Think\Model;
class MibackController extends Controller {
    public function index(){
        ...
        if(stripos("%22",$dataThirdpay)){ //判断接收到的数据是否做过 Urldecode处理，如果没有处理则对数据进行Urldecode处理
            $dataThirdpay= array_map ('urldecode',$dataThirdpay);
        }
        include  JY_ROOT.'thirdpay/Mipay/miapi.php';
        $mipay = new \Miapi();
        //验签
        $signature =  $dataThirdpay['signature'];
        unset($dataThirdpay['signature']);
        $dataSign = '';
        foreach ($dataThirdpay as $k=>$v){
            $dataSign .= $k.'='.$v.'&';
        }
        $dataSign = substr($dataSign,0,strlen($dataSign)-1);

        $parseResp =$mipay->VerificationSign('bFn9ZhgrimiXJ/379DyiyA==',$dataSign, $signature);
        if(!$parseResp){
            $result = 7002;
            goto OrderSave;
        }
        //订单号
        $OrderID        =   $dataThirdpay['cpOrderId'];
        //支付类型
        $paytype        =   0;
        //金额
        $money          =   $dataThirdpay['payFee'];
        //用户ID商品ID
        $appuserid      =  $dataThirdpay['cpUserInfo'];
        ...
        //金额是否先匹配
        if($CatUsersOrderInfo['Price']*100  !=  $money){
            $result = 7001;
            goto OrderSave;
        }
       ...
       //反序列化
        ...
        $dataApiVisitLog = array(
            'Name'=>'支付订单',
            'Url'=>'/Jy_ThirdpayIapppay/Miback/index',
            'Msg'=>$msgArr[$result],
            'Code'=>$result,
            'TimeOut'=>'',
