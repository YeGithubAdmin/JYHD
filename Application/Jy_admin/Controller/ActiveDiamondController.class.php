...
class ActiveDiamondController extends ComController {
    ...
    public function index()
    {
        ...
        $UserComprehensive = M('jy_company as a')
            ->join('jy_user_active_diamond_logo as b on a.id = b.company and '.$whereUserComprehensive,'LEFT')
            ->where($whereUserComprehensive)
            ->field('a.company,a.id,a.name,group_concat(b.num ORDER BY b.DateTime DESC) as num,group_concat(b.DateTime ORDER BY  b.DateTime DESC)')
            ->order('a.end asc')
            ->group('b.company')
            ->select();
    ...
    public function Script(){
            //前天时间
             ...
             $userActiveGoldLogo = M('jy_user_active_diamond_logo')->addAll($UserComprehensive);
    }
