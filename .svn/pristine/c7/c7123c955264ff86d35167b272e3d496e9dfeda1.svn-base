...
function testOrder() {
    ...
    $orderReq['waresid'] = 1;
    $orderReq['cporderid'] = date('Ymd').substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8); //确保该参数每次 都不一样。否则下单会出问题。
    echo "microtime()";
    $orderReq['price'] = 0.01;   //单位：元
    $orderReq['currency'] = 'RMB';
    $orderReq['appuserid'] = '10123059';
    $orderReq['cpprivateinfo'] = '11qwe123r23q232111';
    $orderReq['notifyurl'] = 'http://testphp.juyihd.com/demo/notifyurl.php';
    //组装请求报文  对数据签名
