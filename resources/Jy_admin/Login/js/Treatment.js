...
function AjaxPost(Url,JsonData,LodingFun,ReturnFun) {
    $.ajax({
        ...
        error: function () { AjaxErro({ "Status": "Erro", "Erro": "请求失败服务器错误" }); },
        success: ReturnFun
    });
}
//示例
...
function ErroAlert(e) {
    var index = layer.alert(e, { icon: 5, time: 5000, offset: 't', closeBtn: 0, title: '错误信息', btn: [], anim: 2, shade: 0 });
    layer.style(index, {
        color: '#777'
    }); 
}
...
function AjaxErro(e) {
        ...
        switch (e.Status) {
            case 2:
                ErroAlert("错误 : 该用户被管理锁定，暂时禁止登陆请与管理员联系 '2'");
                break;
            case 3:
                ErroAlert("错误 : 密码错误 '3'");
                break;
            case 4:
                ErroAlert("错误 : 用户不存在 '4'");
                break;
            case 5:
                ErroAlert("错误 : 验证码错误 '5'");
                break;
            default:
                ErroAlert("错误 : 验证码错误 '位置错误'");
        }
